...
BEGIN
    ...
    IF @MASV = N'' OR @HO = N'' OR @TEN = N''
       OR LEN(@MASV) > 8 OR LEN(@HO) > 40 OR LEN(@TEN) > 10 OR LEN(@DIACHI) > 100
       OR @NGAYSINH > CAST(GETDATE() AS DATE)
       OR @MASV COLLATE Latin1_General_BIN2 LIKE N'%[^A-Z0-9]%'
    ...
    IF EXISTS (SELECT 1 FROM dbo.SINHVIEN WHERE RTRIM(MASV) = RTRIM(@MASV))
        RETURN 1;

    IF NOT EXISTS (SELECT 1 FROM dbo.LOP WHERE RTRIM(MALOP) = RTRIM(@MALOP))
        RETURN 2;

    INSERT INTO dbo.SINHVIEN (MASV, HO, TEN, NGAYSINH, DIACHI, MALOP, TRANGTHAI)
    VALUES (@MASV, @HO, @TEN, @NGAYSINH, NULLIF(@DIACHI, N''), @MALOP, 1);
